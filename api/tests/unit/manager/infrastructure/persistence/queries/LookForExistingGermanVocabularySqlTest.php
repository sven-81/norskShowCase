...
#[CoversClass(LookForExistingGermanVocabularySql::class)]
class LookForExistingGermanVocabularySqlTest extends TestCase
{
    ...
    {
        $lookForExistingGermanWordSql = LookForExistingGermanVocabularySql::create($vocabularyType, null);

        self::assertSame(
            'SELECT COUNT("german") AS "match" FROM ' . $vocabularyType->value . 's' . ' WHERE BINARY german = ?;',
    ...
    {
        $lookForExistingGermanWordSql = LookForExistingGermanVocabularySql::create($vocabularyType, Id::by(3));

        self::assertSame(
            'SELECT COUNT("german") AS "match" FROM '
            . $vocabularyType->value . 's'
            . ' WHERE BINARY german = ? AND id != 3;',
            $lookForExistingGermanWordSql->asString()
        );
    }
}
