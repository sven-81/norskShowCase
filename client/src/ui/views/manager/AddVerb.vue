...
watch(norsk, (val) => {
    ...
    if (converted !== val) { norsk.value = converted }
})
watch(norskPresent, (val) => {
    const converted = replaceSpecialChars(val)
    if (converted !== val) { norskPresent.value = converted }
})
watch(norskPast, (val) => {
    const converted = replaceSpecialChars(val)
    if (converted !== val) { norskPast.value = converted }
})
watch(norskPastPerfect, (val) => {
    const converted = replaceSpecialChars(val)
    if (converted !== val) { norskPastPerfect.value = converted }
})

async function onSubmit(verbData: any): Promise<void> {
    try {
        await verbStore.add(verbData)
        const message = `Das Verb "${verbData.norsk}" | "${verbData.german}" wurde hinzugefügt`
  ...
  <Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }">
    ...
    <div class="form-row add-verb">
      <div class="form-group col">
        <Field name="german" type="text" class="form-control" placeholder="Deutsch"
               :class="{ 'is-invalid': errors.german }" />
        <div class="invalid-feedback">{{ errors.german }}</div>
      </div>
      <div class="form-group col">
        <Field name="norsk" type="text" class="form-control" v-model="norsk"
               placeholder="Infinitiv" :class="{ 'is-invalid': errors.norsk }" />
        <div class="invalid-feedback">{{ errors.norsk }}</div>
      </div>
      <div class="form-group col">
        <Field name="norskPresent" type="text" class="form-control" v-model="norskPresent"
               placeholder="Präsens" :class="{ 'is-invalid': errors.norskPresent }" />
        <div class="invalid-feedback">{{ errors.norskPresent }}</div>
      </div>
      <div class="form-group col">
        <Field name="norskPast" type="text" class="form-control" v-model="norskPast"
               placeholder="Vergangenheit" :class="{ 'is-invalid': errors.norskPast }" />
        <div class="invalid-feedback">{{ errors.norskPast }}</div>
      </div>
      <div class="form-group col">
        <Field name="norskPastPerfect" type="text" class="form-control" v-model="norskPastPerfect"
               placeholder="2. Vergangenheit" :class="{ 'is-invalid': errors.norskPastPerfect }" />
